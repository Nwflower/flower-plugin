...
export class singlegacha extends plugin {
  ...
  async gacha () {
    ...
    if (!await this.Coin.useCoin(this.type, 1)){return true}
    await this.getPool()
    await this.userData()
    this.res = this.lottery()
    let recallMsg = this.set.delMsg

    let base64 = await puppeteer.screenshot('gacha', {
      tplFile: `./plugins/flower-plugin/GachaMOD/Genshin/resources/html/single/single.html`,
      /** 绝对路径 */
      pluResPath: `${this._path}/plugins/flower-plugin/GachaMOD/Genshin/resources/`,
      saveId: 'single',
      imgType: 'png',
      name: this.res.name,
      star: this.res.star,
