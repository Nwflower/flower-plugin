...
目前插件已支持通过指令下载抽卡扩展库到 `GachaMOD` 目录。

已支持仓库：[StarRail](https://github.com/jiuyixian/StarRail) ，下载命令 #星铁抽卡下载 或 #星铁抽卡扩展下载 

扩展库下载完成重启，相关功能与锅巴配置项会在检测到目录存在时自动启用；若目标目录已存在，则不会重复下载。  
如需接入新的扩展库，请以仓库内置映射为准。
